...
// SIGALRM handler: stop current, advance, continue next, rearm
static void schedule_handler(int sig) {
    (void)sig;
    if (np <= 1) return;
    kill(pids[current], SIGSTOP);
    current = (current + 1) % np;
    printf("MCP v3.0: scheduling PID %d\n", pids[current]);
...
int main(int argc, char *argv[]) {
    ...
    printf("MCP v3.0: starting PID %d\n", pids[0]);
    kill(pids[0], SIGCONT);
    alarm(1);
    ...
    printf("MCP v3.0: all done, exiting\n");
    return 0;
}
